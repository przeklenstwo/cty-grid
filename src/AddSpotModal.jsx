...
export default function AddSpotModal({ coords, onClose, onAdded, userId }) {
  ...
  function toggleCrew(crewName) {
    setSelectedCrews(prev =>
      prev.includes(crewName) ? prev.filter(c => c !== crewName) : [...prev, crewName]
    )
  }
  ...
  async function handleSubmit() {
    ...
    for (let i = 0; i < images.length; i++) {
      setProgress(`Czyszczenie EXIF i upload ${i + 1}/${images.length}...`)
      try {
    ...
    const { lat, lng } = fuzzLocation(coords.lat, coords.lng, radiusMeters)
    ...
    const { error: insertError } = await supabase.from('spots').insert({
      ...
      is_public: isPublic,
      status: 'approved',
      location_fuzzed: radiusMeters > 0,
      fuzz_radius: radiusMeters,
    })

    if (insertError) { setError(insertError.message); setLoading(false); setProgress(''); return }

    // Pobierz username i wyślij powiadomienie
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px' }}>
          <div>
            <h2 style={{ color: 'white', fontSize: '1.25rem', fontWeight: 700, letterSpacing: '-0.02em' }}>
              🎨 Nowa Praca
            </h2>
            <p style={{ color: '#52525b', fontSize: '0.75rem', marginTop: '2px' }}>
              {radiusMeters > 0
                ? `📍 Lokalizacja rozmyta ~${radiusMeters}m`
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <input style={input} placeholder="Nazwa pracy *" value={title} onChange={e => setTitle(e.target.value)} />

          <textarea
            style={{ ...input, minHeight: '72px', resize: 'vertical' }}
            placeholder="Opis, hashtagi (#wildstyle #throwup)..."
            value={description}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <p style={{ color: '#71717a', fontSize: '0.73rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', margin: 0 }}>
                👥 Crew
              </p>
              <button onClick={() => { setShowNewCrew(s => !s); setCrewError('') }} style={{
                padding: '3px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer',
                background: showNewCrew ? 'rgba(249,115,22,0.18)' : 'rgba(255,255,255,0.06)',
                color: showNewCrew ? '#f97316' : '#71717a',
                fontSize: '0.72rem', fontWeight: 700, fontFamily: 'Space Grotesk, sans-serif',
                outline: showNewCrew ? '1px solid rgba(249,115,22,0.35)' : 'none',
              }}>+ Nowe crew</button>
            </div>
            ...
            {showNewCrew && (
              <div style={{
                ...
                <input
                  ...
                  placeholder="Nazwa crew (np. TKO)"
                  style={{ ...input, marginBottom: '10px', background: 'rgba(255,255,255,0.06)' }}
                  onKeyDown={e => e.key === 'Enter' && handleAddNewCrew()}
                />
                <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '10px' }}>
                  {COLOR_PALETTE.map(c => (
                    <div key={c} onClick={() => setNewCrewColor(c)} style={{
                      ...
                      outlineOffset: '2px',
                      transform: newCrewColor === c ? 'scale(1.2)' : 'scale(1)',
                    }} />
                  ))}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{
                      ...
                      boxShadow: `0 0 10px ${newCrewColor}60`,
                    }}>{newCrewName || 'CREW'}</span>
                  </div>
                  <button onClick={handleAddNewCrew} disabled={addingCrew} style={{
                    padding: '6px 16px', borderRadius: '8px', border: 'none',
                    background: '#f97316', color: 'white', fontWeight: 700,
                    fontSize: '0.8rem', cursor: 'pointer',
                    fontFamily: 'Space Grotesk, sans-serif', opacity: addingCrew ? 0.6 : 1,
                  }}>{addingCrew ? '...' : 'Dodaj ✓'}</button>
                </div>
            ...
            {selectedCrewObjs.length > 0 && (
              <div style={{ marginBottom: '8px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {selectedCrewObjs.map(crew => (
                    <button key={crew.id} onClick={() => toggleCrew(crew.name)} style={{
                      padding: '5px 14px', borderRadius: '9999px', border: 'none',
                      cursor: 'pointer', fontSize: '0.82rem', fontWeight: 700,
                      fontFamily: 'Space Grotesk, sans-serif',
                      background: crew.color, color: '#000',
                      boxShadow: `0 0 12px ${crew.color}55`,
                      display: 'flex', alignItems: 'center', gap: '5px',
                    }}>
                      {crew.name} <span style={{ opacity: 0.6, fontSize: '0.7rem' }}>✕</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {otherCrewObjs.length > 0 && (
              <div>
                {selectedCrewObjs.length > 0 && (
                  <p style={{ color: '#3f3f46', fontSize: '0.68rem', marginBottom: '5px' }}>Dodaj więcej:</p>
                )}
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {otherCrewObjs.map(crew => (
                    <button key={crew.id} onClick={() => toggleCrew(crew.name)} style={{
                      padding: '5px 14px', borderRadius: '9999px', border: 'none',
                      cursor: 'pointer', fontSize: '0.82rem', fontWeight: 700,
                      fontFamily: 'Space Grotesk, sans-serif',
                      background: 'rgba(255,255,255,0.05)', color: crew.color,
                      outline: `1px solid ${crew.color}45`, transition: 'all 0.15s',
                    }}>{crew.name}</button>
                  ))}
                </div>
              </div>
            ...
            {allCrews.length === 0 && !showNewCrew && (
              <p style={{ color: '#3f3f46', fontSize: '0.8rem', textAlign: 'center', padding: '8px 0' }}>
                Brak crew — kliknij "+ Nowe crew" żeby dodać
              </p>
            )}
          </div>
          ...
          {/* Radius */}
          <div>
            <p style={{ color: '#71717a', fontSize: '0.73rem', marginBottom: '7px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              🔒 Precyzja lokalizacji
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
              {RADIUS_OPTIONS.map(opt => (
                <button key={opt.value} onClick={() => setRadiusMeters(opt.value)} style={{
                  padding: '8px 4px', borderRadius: '8px', border: 'none',
                  background: radiusMeters === opt.value ? '#f97316' : 'rgba(255,255,255,0.05)',
                  color: radiusMeters === opt.value ? 'white' : '#71717a',
            ...
            {radiusMeters > 0 && (
              <p style={{ color: '#52525b', fontSize: '0.7rem', marginTop: '5px' }}>
                ⚠️ Pinezka pojawi się w promieniu {radiusMeters}m od klikniętego punktu
              </p>
            )}
          </div>
          ...
          <div>
            <label style={{
              ...
              border: '1px dashed rgba(249,115,22,0.35)', cursor: 'pointer',
              color: '#f97316', fontSize: '0.85rem', background: 'rgba(249,115,22,0.04)',
            }}>
              📷 Dodaj zdjęcia (max 8) — GPS/EXIF usuwane automatycznie
                ...
                {previews.map((url, i) => (
                  <div key={i} style={{ position: 'relative' }}>
                    <img src={url} alt="" style={{
                      ...
                      border: i === 0 ? '2px solid #f97316' : '1px solid rgba(255,255,255,0.08)',
                    }} />
                    {i === 0 && (
                      <span style={{
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ color: '#52525b', fontSize: '0.73rem', marginTop: '1px' }}>
                {isPublic ? 'Widoczna dla wszystkich' : 'Tylko Ty ją widzisz'}
              </p>
            </div>
            <div onClick={() => setIsPublic(!isPublic)} style={{
              width: '44px', height: '24px', borderRadius: '9999px',
              background: isPublic ? '#f97316' : '#27272a',
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
            <button onClick={onClose} style={{
              ...
              color: '#71717a', cursor: 'pointer', fontWeight: 600,
              fontFamily: 'Space Grotesk, sans-serif',
            }}>Anuluj</button>
            <button onClick={handleSubmit} disabled={loading} style={{
              flex: 2, padding: '12px', borderRadius: '10px', border: 'none',
              background: loading ? '#7c3d12' : '#f97316', color: 'white', fontWeight: 700,
              cursor: loading ? 'not-allowed' : 'pointer',
              fontFamily: 'Space Grotesk, sans-serif',
            }}>
              {loading ? 'Przetwarzanie...' : 'Dodaj Pracę 🎨'}
            </button>
          </div>
        </div>
      </div>
