...
export default function SpotModal({ spot, userId, userRank = 0, isAdmin, onClose, onDeleted, onRefresh }) {
  ...
  const [comments, setComments]           = useState([]) // tylko top-level
  const [replies, setReplies]             = useState({}) // { parent_id: [replies] }
  ...
  async function fetchComments() {
    ...
    // Rozdziel na top-level i replies
  ...
  function CommentTypeBadge({ type }) {
    ...
    return (
      <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '2px 6px', borderRadius: '4px', background: m.bg, color: m.color }}>
        {m.label}
      </span>
    )
  }
      ...
      <div style={{ marginBottom: isReply ? '6px' : '10px' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3px', gap: '6px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <button onClick={() => goToProfile(c.user_id)} style={{
                background: 'none', border: 'none', padding: 0, cursor: 'pointer',
                color: rankColors[c.profiles?.rank ?? 0], fontWeight: 700, fontSize: '0.8rem',
                fontFamily: 'Space Grotesk, sans-serif',
              }}>{c.profiles?.username || 'Anonim'}</button>
              <CommentTypeBadge type={c.comment_type} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ color: '#3f3f46', fontSize: '0.68rem' }}>
                {new Date(c.created_at).toLocaleDateString('pl-PL')}
              </span>
              {/* Przycisk odpowiedzi — tylko dla top-level */}
              {!isReply && (
                <button
                  onClick={() => setReplyingTo(replyingTo?.id === c.id ? null : { id: c.id, username: c.profiles?.username || 'Anonim' })}
                  style={{
        ...
        {!isReply && replyingTo?.id === c.id && (
          <div style={{
            ...
            <div style={{
              width: '2px', background: 'rgba(249,115,22,0.4)',
              alignSelf: 'stretch', borderRadius: '1px', flexShrink: 0,
            }} />
            <div style={{ flex: 1, display: 'flex', gap: '6px' }}>
              <span style={{ color: '#f97316', fontSize: '0.75rem', alignSelf: 'center', whiteSpace: 'nowrap' }}>
                @{replyingTo.username}
              </span>
              <input
                autoFocus
                ...
                onKeyDown={e => { if (e.key === 'Enter') handleSendReply(); if (e.key === 'Escape') setReplyingTo(null) }}
              ...
              />
              <button onClick={handleSendReply} disabled={replyLoading || !replyText.trim()} style={{
        ...
        {!isReply && commentReplies.length > 0 && (
          <div style={{ marginLeft: '16px', marginTop: '4px', display: 'flex', gap: '8px' }}>
            ...
            <div style={{ flex: 1 }}>
              {showReplies ? (
                <>
                  <button onClick={() => setShowReplies(false)} style={{
                    background: 'none', border: 'none', color: '#52525b',
                    fontSize: '0.7rem', cursor: 'pointer', padding: '2px 0 6px',
                    fontFamily: 'Space Grotesk, sans-serif',
                  }}>▲ ukryj odpowiedzi ({commentReplies.length})</button>
                ...
                }}>▼ {commentReplies.length} {commentReplies.length === 1 ? 'odpowiedź' : 'odpowiedzi'}</button>
              )}
            </div>
          </div>
        )}
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        height: 'min(56.25vw, 82vh)', display: 'flex', flexDirection: 'row',
            ...
            {userRank < 2 && !isAdmin && (
              <div style={{ marginBottom: '10px', padding: '7px 10px', borderRadius: '8px', background: 'rgba(167,139,250,0.05)', border: '1px solid rgba(167,139,250,0.12)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <p style={{ color: '#71717a', fontSize: '0.72rem', margin: 0 }}>
                  Komentarze o <strong style={{ color: '#a78bfa' }}>przypale</strong> od rangi <strong style={{ color: '#a78bfa' }}>Veteran</strong>
                </p>
              </div>
            )}
          ...
          <div style={{ padding: '10px 20px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            {sent ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '5px' }}>
                    {availableTypes.map(t => (
                      <button key={t.value} onClick={() => setCommentType(t.value)} style={{
                        ...
                        fontSize: '0.7rem', fontWeight: 600, fontFamily: 'Space Grotesk, sans-serif',
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input
                    style={{ flex: 1, padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.04)', color: 'white', fontSize: '0.88rem', fontFamily: 'Space Grotesk, sans-serif', outline: 'none' }}
