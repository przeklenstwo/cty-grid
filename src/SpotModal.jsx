...
export default function SpotModal({ spot, userId, userRank = 0, isAdmin, onClose, onDeleted, onRefresh }) {
  ...
  function CommentTypeBadge({ type }) {
    ...
    return (
      <span style={{
        fontSize: isMobile ? '0.64rem' : '0.68rem', fontWeight: 700,
        padding: isMobile ? '2px 6px' : '2px 7px', borderRadius: '4px',
  ...
  const pad = isMobile ? '14px 14px 12px' : '20px 24px 14px'
  ...
  return (
    <div onClick={onClose} style={{
      ...
      padding: isMobile ? '10px' : '24px', fontFamily: 'Space Grotesk, sans-serif',
    }}>
      <div onClick={e => e.stopPropagation()} style={{
            ...
            <>
              ...
              {isBuffed && (
                <div style={{
                  ...
                  background: 'rgba(0,0,0,0.8)', borderRadius: '8px', padding: '4px 10px',
                  display: 'flex', alignItems: 'center', gap: '5px',
                }}>
                  <span>🪣</span>
                  <span style={{ color: '#71717a', fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.1em' }}>BUFFED</span>
                </div>
              )}
              <div style={{
              ...
              {imageList.length > 1 && (
                <>
                  <button onClick={() => setCurrentImg(p => Math.max(0, p - 1))} style={{
                    position: 'absolute', left: isMobile ? '10px' : '14px', top: '50%', transform: 'translateY(-50%)',
                    background: 'rgba(0,0,0,0.65)', border: 'none', color: 'white', borderRadius: '50%',
                    width: isMobile ? '36px' : '40px', height: isMobile ? '36px' : '40px',
                    cursor: 'pointer', fontSize: isMobile ? '1.2rem' : '1.4rem', lineHeight: 1,
                  }}>‹</button>
                  <button onClick={() => setCurrentImg(p => Math.min(imageList.length - 1, p + 1))} style={{
                    position: 'absolute', right: isMobile ? '10px' : '14px', top: '50%', transform: 'translateY(-50%)',
                    background: 'rgba(0,0,0,0.65)', border: 'none', color: 'white', borderRadius: '50%',
                    width: isMobile ? '36px' : '40px', height: isMobile ? '36px' : '40px',
                    cursor: 'pointer', fontSize: isMobile ? '1.2rem' : '1.4rem', lineHeight: 1,
                  }}>›</button>
        ...
        <div style={{
          ...
          <div style={{ padding: pad, flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
              <h2 style={{
                ...
                letterSpacing: '-0.02em', lineHeight: 1.15, margin: 0, flex: 1,
            ...
            {/* AUTOR */}
            {author && (
              <button onClick={() => goToProfile(author.id)} style={{
                marginTop: '10px', display: 'flex', alignItems: 'center', gap: '7px',
                background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)',
                borderRadius: '8px', padding: '6px 12px', cursor: 'pointer',
                fontFamily: 'Space Grotesk, sans-serif', transition: 'all 0.15s', width: 'fit-content',
              }}
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
              {isBuffed && (
                <span style={{ padding: '2px 10px', borderRadius: '9999px', fontSize: '0.7rem', fontWeight: 700, background: 'rgba(113,113,122,0.15)', color: '#71717a', border: '1px solid rgba(113,113,122,0.3)' }}>🪣 BUFFED</span>
              )}
              <span style={{ color: '#52525b', fontSize: isMobile ? '0.68rem' : '0.73rem' }}>
                📌 {spot.location_fuzzed ? `~${spot.fuzz_radius}m` : `${spot.lat?.toFixed(4)}, ${spot.lng?.toFixed(4)}`}
              </span>
              <span style={{ color: spot.is_public ? '#22c55e' : '#f97316', fontSize: isMobile ? '0.68rem' : '0.73rem' }}>
            ...
            {spot.description && (
              <p style={{ color: isBuffed ? '#3f3f46' : '#a1a1aa', fontSize: isMobile ? '0.8rem' : '0.87rem', lineHeight: 1.55, marginTop: '10px', marginBottom: 0, wordBreak: 'break-word' }}>
            ...
            {/* Confirm delete */}
            {confirmDelete && (
              <div style={{ marginTop: '10px', padding: isMobile ? '10px' : '12px', borderRadius: '10px', background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.2)' }}>
                <p style={{ color: '#f87171', fontSize: isMobile ? '0.78rem' : '0.83rem', marginBottom: '8px', marginTop: 0, fontWeight: 600 }}>⚠️ Usunąć tę pracę? Nie można cofnąć.</p>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => setConfirmDelete(false)} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.08)', background: 'none', color: '#a1a1aa', cursor: 'pointer', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: isMobile ? '0.78rem' : '0.82rem' }}>Anuluj</button>
                  <button onClick={handleDelete} disabled={deleting} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: 'none', background: '#ef4444', color: 'white', cursor: deleting ? 'not-allowed' : 'pointer', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: isMobile ? '0.78rem' : '0.82rem', opacity: deleting ? 0.6 : 1 }}>{deleting ? 'Usuwanie...' : '🗑 Tak, usuń'}</button>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Komentarze */}
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: isMobile ? '12px 14px' : '14px 24px' }}>
            <p style={{ color: '#3f3f46', fontSize: isMobile ? '0.64rem' : '0.68rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: 0, marginBottom: '10px' }}>
            ...
            {userRank < 2 && !isAdmin && (
              <div style={{ marginBottom: '12px', padding: '8px 12px', borderRadius: '8px', background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.15)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <p style={{ color: '#71717a', fontSize: isMobile ? '0.72rem' : '0.75rem', margin: 0, lineHeight: 1.4 }}>
                  Komentarze o <strong style={{ color: '#a78bfa' }}>przypale</strong> widoczne od rangi <strong style={{ color: '#a78bfa' }}>Veteran</strong>
                </p>
              </div>
            )}
            {comments.length === 0 && (
              <p style={{ color: '#3f3f46', fontSize: isMobile ? '0.8rem' : '0.85rem' }}>Brak komentarzy. Bądź pierwszy!</p>
            )}
            {comments.map(c => (
              <div key={c.id} style={{
                marginBottom: '10px', padding: isMobile ? '10px 12px' : '10px 14px', borderRadius: '10px',
                background: c.comment_type === 'hazard' ? 'rgba(239,68,68,0.05)' : c.comment_type === 'buff_report' ? 'rgba(113,113,122,0.07)' : c.comment_type === 'tip' ? 'rgba(234,179,8,0.04)' : 'rgba(255,255,255,0.03)',
                border: c.comment_type === 'hazard' ? '1px solid rgba(239,68,68,0.15)' : c.comment_type === 'buff_report' ? '1px solid rgba(113,113,122,0.2)' : c.comment_type === 'tip' ? '1px solid rgba(234,179,8,0.12)' : '1px solid rgba(255,255,255,0.05)',
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isSmallMobile ? 'flex-start' : 'center', marginBottom: '6px', gap: '8px', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                    <button onClick={() => c.profiles?.username && goToProfile(c.user_id)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: rankColors[c.profiles?.rank ?? 0], fontWeight: 600, fontSize: isMobile ? '0.78rem' : '0.82rem', fontFamily: 'Space Grotesk, sans-serif' }}>
                      {c.profiles?.username || 'Anonim'}
                    </button>
                    <CommentTypeBadge type={c.comment_type} />
                  </div>
                  <span style={{ color: '#3f3f46', fontSize: isMobile ? '0.66rem' : '0.7rem', flexShrink: 0 }}>
                    {new Date(c.created_at).toLocaleDateString('pl-PL')}
                  </span>
                </div>
                <p style={{ color: '#d4d4d8', fontSize: isMobile ? '0.8rem' : '0.85rem', lineHeight: 1.5, margin: 0, wordBreak: 'break-word' }}>{c.content}</p>
              </div>
            ))}
          </div>

          {/* Input */}
          <div style={{ padding: isMobile ? '12px 14px 14px' : '12px 24px 18px', borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            {sent ? (
              <div style={{ background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '10px', padding: '10px 14px', color: '#22c55e', fontSize: isMobile ? '0.8rem' : '0.85rem', textAlign: 'center' }}>
                ...
                {availableTypes.length > 1 && (
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                    {availableTypes.map(t => (
                      <button key={t.value} onClick={() => setCommentType(t.value)} style={{ padding: isMobile ? '5px 9px' : '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: isMobile ? '0.68rem' : '0.72rem', fontWeight: 600, fontFamily: 'Space Grotesk, sans-serif', background: commentType === t.value ? 'rgba(249,115,22,0.2)' : 'rgba(255,255,255,0.04)', color: commentType === t.value ? '#f97316' : '#71717a', outline: commentType === t.value ? '1px solid rgba(249,115,22,0.4)' : 'none' }}>{t.label}</button>
                    ))}
                  </div>
                )}
                <div style={{ display: 'flex', flexDirection: isSmallMobile ? 'column' : 'row', gap: '8px' }}>
                  <input
                    style={{ flex: 1, minWidth: 0, padding: isMobile ? '11px 12px' : '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.04)', color: 'white', fontSize: isMobile ? '0.84rem' : '0.88rem', fontFamily: 'Space Grotesk, sans-serif', outline: 'none' }}
                    placeholder="Napisz komentarz..."
                    value={newComment}
                    onChange={e => setNewComment(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && handleSendComment()}
                  />
                  <button onClick={handleSendComment} disabled={loading || !newComment.trim()} style={{ width: isSmallMobile ? '100%' : 'auto', padding: isMobile ? '11px 16px' : '10px 18px', borderRadius: '10px', border: 'none', background: '#f97316', color: 'white', fontWeight: 700, cursor: 'pointer', fontFamily: 'Space Grotesk, sans-serif', opacity: !newComment.trim() ? 0.4 : 1, flexShrink: 0 }}>
                    {isSmallMobile ? 'Wyślij' : '→'}
                  </button>
                </div>
              </div>
            )}
